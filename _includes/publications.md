...
<li style="margin-bottom:1rem">
  <div class="col-sm-9">
      ...
      {% else %}
        {% if link.arxiv %} 
          <div class="title"><b>{{ link.title }}</b>, {{ link.status }} ({{ link.year }}). </div>
        {% else %}
      ...
      {% endif %}
      {% if link.arxiv %} 
      <a href="{{ link.arxiv }}" class="button" style="font-size:16px;"><i class="ai ai-arxiv" aria-hidden="true"></i>&nbsp; arXiv</a>
      {% endif %}
      {% if link.code %} 
